...
public class ExerciseRepository {
    ...
    @Inject
    public ExerciseRepository(UltimateFitDatabase db, ExerciseDao exerciseDao, UltimateFitService ultimateFitService, Context context) {
        this.db = db;
    ...
    public LiveData<Resource<List<Exercise>>> getExercises(int categoryPk) {
        return new NetworkBoundResource<List<Exercise>, ExerciseApiResponse>() {
            @Override
            ...
            protected LiveData<ApiResponse<ExerciseApiResponse>> createCall() {
                ...
                long lastUpdated = SharedPreferenceHelper.getInstance(context).getLong(SharedPreferenceHelper.Key.LAST_EXERCISE_MODIFIED_DATE_LONG);
                if (lastUpdated == 0) {
                    lastUpdated = 1000000;
                }
